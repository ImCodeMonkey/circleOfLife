<template>
	<view>
		<view class="header" v-bind:class="{'status':isH5Plus}">
			<view class="userinfo">
				<view class="face"><image :src="userinfo.avatarUrl" @tap="logins()"></image></view>
				<view class="info">
					<view class="username">{{userinfo.nickName}}</view>
					<view class="autograph">{{userinfo.autograph?userinfo.autograph:'永远相信美好的事情即将发生'}}</view>
				</view>
			</view>
			<view class="setting" @tap="setting()"><image src="../../../static/userInfo/setting.png"></image></view>
		</view>
		<view class="orders">
			<view class="box">
	...
	export default {
		...
		methods: {
			init() {
				...
				//用户信息	
				if(typeof(this.userInformation.avatarUrl) != 'undefined'){
					this.userinfo = this.userInformation
				}else{
					this.userinfo = {
						avatarUrl: '../../../static/userInfo/face.png',
						nickName: '',
						autograph: '点击左侧头像登录',
					}
				}
			...
			logins(e) {
				//未登陆状态下点击才会登录
				if(typeof(this.userInformation.avatarUrl) === 'undefined'){
					let href = '/pages/loginAuthorization/login/login';
					if(href){
						uni.navigateTo({
							url: href
						});
			...
			setting() {
				uni.navigateTo({
					url: '/pages/personalCenter/personalSettings/personalSettings'
				});
			}
		}
	}
